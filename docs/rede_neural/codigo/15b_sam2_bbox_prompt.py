"""
15b_sam2_bbox_prompt.py
=========================
SAM2 com BBOX prompt nos recortes GeoTIFF.
IMPORTANTE: SAM2 carregado ANTES de rasterio/geopandas (conflito de DLLs).
...
OUTPUT_DIR = os.path.join(BASE_DIR, f"resultado_sam2_bbox_{CROP_SIZE}")
...
# ── CARREGAR SAM2 PRIMEIRO (antes de rasterio) ───────────────
...
_sam2_model.eval()
print("  SAM2 carregado!", flush=True)
...
# ── SAM2 BBOX SEGMENT ────────────────────────────────────────
@torch.no_grad()
def sam2_bbox_segment(pil_img, bbox_xyxy):
    try:
        input_boxes = [[bbox_xyxy]]  # 3 níveis: [image, box, coords]
        inputs = _sam2_processor(
            images=pil_img, input_boxes=input_boxes,
            return_tensors="pt").to(DEVICE)

        outputs = _sam2_model(**inputs)

        pred_masks = outputs.pred_masks.cpu().numpy()  # (1,1,3,256,256)
        iou_scores = outputs.iou_scores.cpu().numpy()  # (1,1,3)

        masks_3 = pred_masks[0, 0]  # (3, 256, 256)
        scores_3 = iou_scores[0, 0]  # (3,)
        ...
        max_d = ((orig_w / 2)**2 + (orig_h / 2)**2)**0.5
        ...
        for i, mask in enumerate(masks_resized):
            ...
            combined = float(scores_3[i]) * (max(0.0, 1.0 - d) ** 2.0)
            if combined > best_val:
                best_val = combined
                best_idx = i

        return masks_resized[best_idx], float(scores_3[best_idx])

    except Exception as e:
        print(f"    [SAM2 ERRO] {e}", flush=True)
        return None, 0.0
    finally:
...
def save_visual(rgb, mask_bool, score, bbox_xyxy, base_name, visual_dir):
    ...
    if mask_bool is not None and mask_bool.any():
        overlay[mask_bool] = (overlay[mask_bool].astype(np.float32) * 0.55
            + np.array([50, 100, 220], dtype=np.float32) * 0.45).astype(np.uint8)
        contours, _ = cv2.findContours(mask_bool.astype(np.uint8),
            cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cv2.drawContours(overlay, contours, -1, (255, 255, 255), 2)
    cv2.putText(overlay, f"SAM2 bbox | {score:.3f}", (10, 22),
    ...
    cv2.imwrite(os.path.join(visual_dir, f"sam2bbox_{base_name}.png"), panel[:, :, ::-1])


# ── MAIN ──────────────────────────────────────────────────────
def main():
    print("=" * 60, flush=True)
    print(f"  SAM2 BBOX PROMPT — {CROP_SIZE}", flush=True)
    ...
    for idx, tif_name in enumerate(tif_files):
        ...
        try:
            ...
            mask_raw, score = sam2_bbox_segment(pil_img, bbox)
            ...
            results.append({"geometry": polygon, "id": meta.get("id", 0),
                ...
                "sam2_score": round(score, 4), "filename": tif_name})
    ...
    if results and crs:
        ...
        gpkg = os.path.join(OUTPUT_DIR, f"cecropia_SAM2_bbox_{CROP_SIZE}.gpkg")
    ...
    score_m = np.mean([r["sam2_score"] for r in results]) if results else 0

    print("\n" + "=" * 60, flush=True)
    print(f"  SAM2 BBOX — {CROP_SIZE} — CONCLUIDO", flush=True)
    print("=" * 60, flush=True)
