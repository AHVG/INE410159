"""
09_encontrar_nao_anotadas_train.py
====================================
Encontra as detecções do YOLO no conjunto de TREINO que NÃO têm
label correspondente (possíveis cecropias não anotadas).

Mesmo processo dos scripts 09 para test e val, agora para train.
ATENÇÃO: São 776 imagens — a curadoria visual levará mais tempo.
...
MODEL_PATH = r"F:/MESTRADO_Bilada/01_Mestrado/GEO/vetor/Bbox_cecropia/model_v1/yolov8x_det_final_AdamW/weights"
TRAIN_IMAGES_DIR = r"F:/MESTRADO_Bilada/01_Mestrado/GEO/vetor/Bbox_cecropia/dataset/images/train"
TRAIN_LABELS_DIR = r"F:/MESTRADO_Bilada/01_Mestrado/GEO/vetor/Bbox_cecropia/dataset/labels/train"
OUTPUT_DIR = r"F:/MESTRADO_Bilada/01_Mestrado/GEO/vetor/Bbox_cecropia/nao_anotadas_train_v2"
...
def main():
    print("=" * 60)
    print("ENCONTRAR CECROPIAS NÃO ANOTADAS NO TRAIN")
    print("=" * 60)
    print(f"  Modelo: {MODEL_PATH}")
    print(f"  Imagens train: {TRAIN_IMAGES_DIR}")
    print(f"  Labels train: {TRAIN_LABELS_DIR}")
    ...
    sample_labels = [f for f in os.listdir(TRAIN_LABELS_DIR) if f.endswith('.txt')]
    if len(sample_labels) > 0:
        with open(os.path.join(TRAIN_LABELS_DIR, sample_labels[0])) as f:
            sample_line = f.readline().strip()
            n_values = len(sample_line.split())
            print(f"  Formato detectado: {n_values} valores por linha (polígono com {(n_values-1)//2} pontos)")

    # =========================================================================
    # 2. Listar imagens de train
    # =========================================================================
    print("\n[2/4] Listando imagens de treino...")
    ...
    imagens_sem_nao_anotadas = 0

    csv_path = os.path.join(OUTPUT_DIR, "relatorio_nao_anotadas_train.csv")
    ...
    print(f"  Imagens sem não anotadas: {imagens_sem_nao_anotadas}")
    print(f"\n  Saídas:")
    ...
    print(f"    3. Rode o merge para train")
    print(f"    4. Retreine o modelo com labels corrigidos (train + val + test)")
    print("=" * 60)
