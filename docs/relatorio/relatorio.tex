...
% Título de \part em uma linha só ("Parte N - Título"), tudo no tamanho \huge do
% título (o rótulo "Parte N" deixa de ser menor que o título).
\titleformat{\part}[block]
  {\normalfont\huge\bfseries\raggedright}
  {\partname~\thepart\ -}{0.5em}{}
\titlespacing*{\part}{0pt}{3.5ex plus 1ex minus .2ex}{2.5ex plus .2ex}
...
\title{
    ...
    \large INE410159 — Visão Computacional \\ UFSC
}
\author{Grupo 10 \\[0.3em]
\small Augusto de H. V. Guerner \quad Eduardo G. de Lazari \quad Fábio H. A. Coelho\\
\small Gabriel Arantes F. Gualda \quad Nena A. Impanta}
\date{Julho de 2026}
% ─────────────────────────────────────────────────────────────────────────────
...
Tamanho aproximado em disco (\texttt{data/tiles}) & 1,4 GB \\
...
\texttt{<nome>\_vis.png}). Atualmente há 50 JSONs de validação, todos revisados
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{../../data/output/tiles/tile_0681/grid.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\textwidth]{../../data/output/tiles/tile_0681/2_gaussian_blur_9x9.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../data/output/tiles/tile_0681/3_canal_h.png}
        \caption{Canal H (matiz).}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../data/output/tiles/tile_0681/4_canal_s.png}
        \caption{Canal S (saturação).}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../data/output/tiles/tile_0681/5_canal_v.png}
        \caption{Canal V (brilho).}
    \end{subfigure}
    \caption{Canais H, S e V extraídos após a conversão para HSV.}
    \label{fig:etapa-hsv}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\textwidth]{../../data/output/tiles/tile_0681/6_mascara_hsv_bruta.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\textwidth]{../../data/output/tiles/tile_0681/7_close_25x25.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\textwidth]{../../data/output/tiles/tile_0681/8_open_9x9.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\textwidth]{../../data/output/tiles/tile_0681/9_filtro_forma_tamanho.png}
...
\subsection{Etapa 7 - Filtragem de Contornos (Área e Forma)}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\textwidth]{../../data/output/tiles/tile_0681/9_filtro_forma_tamanho.png}
    \caption{Candidatos após a filtragem por área mínima e pela regra final de
             forma e tamanho.}
    \label{fig:etapa-filtro-final}
\end{figure}

\textbf{Motivação.} A segmentação por cor é propositalmente permissiva, então
esta etapa reúne dois critérios geométricos que separam copa de ruído. Primeiro,
um limiar de área mínima descarta regiões muito pequenas — ruído, bordas de
outras plantas ou pequenos agrupamentos de folhas. Em seguida, uma regra de
forma e tamanho rejeita parte dos falsos positivos que sobreviveram, como centros
de palmeiras e blobs fundidos grandes demais.

Contornos com área inferior a $10.000\;\text{px}^2$ são descartados, eliminando
manchas pequenas e ruído residual das etapas morfológicas. Sobre os candidatos
restantes aplica-se a regra final: um contorno é aceito como embaúba quando sua
área é maior ou igual a $33.798\;\text{px}^2$ ou sua circularidade é menor ou
igual a $0,1551$, desde que a área não ultrapasse $600.000\;\text{px}^2$ e a
solidez seja pelo menos $0,48$. A primeira condição favorece copas grandes; a
segunda captura formas menos circulares; o teto de área remove blobs fundidos
gigantes; e a solidez evita regiões excessivamente fragmentadas.

\subsection{Etapa 8 - Convex Hull}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\textwidth]{../../data/output/tiles/tile_0681/10_convex_hull_final.png}
...
tile\_0328.jpg & 11 &   970.956 & 69,81 & 28,60\% \\
...
tile\_0143.jpg & 10 &   854.036 & 61,41 & 19,45\% \\
tile\_0175.jpg & 10 &   699.474 & 50,29 & 16,76\% \\
\bottomrule
\end{tabular}
\end{table}
...
A validação compara a saída final do detector com a revisão manual de 50 tiles.
...
A precisão de 86,5\% indica que a maior parte das regiões detectadas corresponde
a embaúba. O recall de 69,4\% mostra que o método ainda perde copas reais,
principalmente quando a segmentação HSV não produz um candidato inicial ou quando
a copa aparece com iluminação e cor fora da faixa definida.
...
O tempo médio de 436,2 ms por tile demonstra que o pipeline clássico permanece
eficiente para inspeção em lote. O total de 590 tiles válidos foi processado em
208,0 s, com uso de memória estável ($\sim$64 MB/tile), reflexo das operações
...
\subsection{Alta taxa de tiles com detecções (85,9\%)}

O fato de 507 em 590 tiles apresentarem ao menos uma detecção sugere cobertura
...
A área máxima atual foi limitada a 599.406 px² ($\approx$ 43,10 m²) pelo teto de
...
A validação final mostra 179 verdadeiros positivos, 28 falsos positivos e 79
falsos negativos em 50 tiles revisados. A precisão de 86,5\% indica que o filtro final de tamanho,
circularidade e solidez reduziu bem os falsos positivos. O
recall de 69,4\%, por outro lado, mostra que parte das copas reais ainda não entra
...
A metodologia proposta é composta por seis etapas sequenciais:
...
em vermelho, permitindo ao operador avaliar cada caso.
...
Custo computacional & Baixo ($\sim$436 ms/tile, CPU) & Alto (GPU; otimização + treino de 500 épocas) \\
...
Resultado & P 86,5\% / R 69,4\% / F1 77,0\% & 1.226 indivíduos mapeados; taxa global 54,3\% \\
\bottomrule
\end{tabular}
\end{table}
...
fica limitado (69,4\%) e há fusão de copas próximas. A abordagem neural procura
...
Foram detectadas 2.003 regiões em 590 tiles válidos, e a validação manual atingiu
86,5\% de precisão e 69,4\% de recall. O filtro final reduz fortemente os falsos
...
Os artefatos que acompanham este trabalho estão disponíveis publicamente nos
seguintes endereços:

\begin{itemize}
    \item \textbf{Repositório (GitHub):}
    \url{https://github.com/AHVG/INE410159}
    \item \textbf{Tiles do ortomosaico (Google Drive):}
    \url{https://drive.google.com/drive/folders/18Mxj-XuCmeZhhcYMINwV4S6uDQdYM0_K}
    \item \textbf{Apresentação (Google Slides):}
    \url{https://docs.google.com/presentation/d/193hvQEPbfMTE8-Aez9RW_OOVceWkpO58VYbRgtGM1_0/edit?usp=sharing}
    \item \textbf{Vídeo de apresentação (YouTube):}
    \url{https://www.youtube.com/watch?v=eB7FDu5-tiM}
\end{itemize}

\section{Declaração de Uso de Inteligência Artificial}

O uso de inteligência artificial neste trabalho ocorreu em duas frentes distintas.

\textbf{IA como método (Parte II).} A abordagem neural é construída sobre dois
modelos:

\begin{itemize}
    \item \textbf{SAM 3} (Meta AI): geração do dataset por \textit{prompt} textual
    (``tree'') e segmentação final das copas por \textit{prompt} geométrico
    (\textit{bounding box}).
    \item \textbf{YOLOv8x} (Ultralytics): detecção e localização dos indivíduos de
    \textit{Cecropia}.
\end{itemize}

A otimização dos hiperparâmetros do YOLOv8x usou o \textit{framework} Optuna
(amostrador TPE) e a análise de importância das variáveis, um regressor
\textit{Random Forest}.

\textbf{IA como assistente de desenvolvimento.} O assistente \textbf{Claude}
(Anthropic), por meio do Claude Code, foi utilizado no desenvolvimento do projeto
para refatorar e organizar o código do pipeline clássico, gerar as figuras de
passo a passo e de estatísticas e redigir e revisar o texto deste relatório e dos
slides. Todas as saídas geradas com auxílio de IA foram revisadas e validadas
pelos autores, que mantêm a responsabilidade final pelo conteúdo.
