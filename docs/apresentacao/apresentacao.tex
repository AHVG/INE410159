...
\begin{frame}{Visão Geral do Pipeline}
    \centering
    \includegraphics[width=0.91\textwidth]{../../data/output/passo_a_passo/tile_0634.png}
\end{frame}
...
\begin{frame}{Métricas e Resultados (Abordagem Neural)}
    \textbf{Métricas de avaliação:} Precisão, Recall, F1, AP, mAP50 e mAP50-95
    (IoU de 0,5 para VP; uma única classe).

    \vspace{0.6em}
    \begin{block}{Resultados quantitativos}
        \textit{\textcolor{red}{[Pendente]}} O material da frente neural encerra em
        ``Métricas de avaliação''. A preencher: mAP50/mAP50-95, curvas
        Precision-Recall e de treino, importância dos hiperparâmetros, contagem final
        de indivíduos mapeados e figuras qualitativas de detecção/segmentação.
    \end{block}
\end{frame}
...
\begin{frame}{Comparação das Duas Abordagens}
    \begin{table}
        ...
        \begin{tabular}{lll}
            ...
            Custo & Baixo ($\sim$152 ms/tile, CPU) & Alto (GPU, 500 épocas) \\
            Segmentação & Fecho convexo & Máscara SAM 3 \\
            Discriminação & Fraca (espectral) & Potencialmente alta \\
            Resultado & P 84,6 / R 52,4 / F1 64,7 & \textcolor{red}{a definir} \\
            \bottomrule
        \end{tabular}
    \end{table}
    \vspace{0.3em}
...
\begin{frame}{Conclusões e Trabalhos Futuros}
    \begin{itemize}
        ...
        primeira estimativa espacial: 1.218 regiões, 84,6\% de precisão, 52,4\% de recall.
        \item O pipeline \textbf{neural} reduz a anotação via SAM 3 e evita vazamento com
        divisão espacial; falta consolidar os resultados quantitativos.
        \item \textbf{Próximos passos (clássica)}: refinar HSV, consolidar detecções entre
        tiles, testar descritores de textura.
        \item \textbf{Próximos passos (neural)}: concluir a avaliação, estender a correção
        ao treino e testar replicabilidade para outras espécies.
