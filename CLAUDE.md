...
- O núcleo fica em `src/core/` (`deteccao.py`, `execucao.py`, `estatisticas.py`) e usa imports relativos entre si. As únicas entradas públicas são `main.py` (pipeline completo sem path, JSON por imagem/pasta com path) e `anotar.py` (UI OpenCV de revisão).
